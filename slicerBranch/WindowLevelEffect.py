...
comment = """
  ...
  WindowLevelEffect is a subclass of LabelEffect
...
#
# WindowLevelEffectOptions - see LabelEffect, EditOptions and Effect for superclasses
#
...
class WindowLevelEffectOptions(LabelEffect.LabelEffectOptions):
  ...
  def __init__(self, parent=0):
    print('Initializing windowLevelEffect')
...
class WindowLevelEffectTool(LabelEffect.LabelEffectTool):
  ...
  def apply(self):
    ...
    if bgNode:
      self.updateWindowLevel(bgLayer, self.polyData.GetBounds())
    if fgNode:
      self.updateWindowLevel(fgLayer, self.polyData.GetBounds())
    self.polyData.GetPoints().Modified()

  def updateWindowLevel(self, sliceLayer, xyBounds):
...
#
# The WindowLevelEffect class definition 
#

class WindowLevelEffect(LabelEffect.LabelEffect):
  ...
  def __init__(self):
    ...
    self.toolTip = "WindowLevel: circular paint brush for label map editing"
...
import EditorLib
pet = EditorLib.WindowLevelEffectTool(sw)
